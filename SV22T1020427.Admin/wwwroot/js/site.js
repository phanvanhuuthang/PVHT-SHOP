...
function paginationSearch(event, form, page) {
    ...
    // page: dùng set để không bị trùng tham số
    formData.set("page", page);

    // Chuẩn hóa các ô tiền (AutoNumeric: 5.000.000 -> 5000000)
    form.querySelectorAll(".money-input[name]").forEach(input => {
        ...
        // bỏ dấu phân tách hàng nghìn, đổi dấu thập phân về '.':
        const normalized = raw.replace(/\./g, "").replace(",", ".");
        formData.set(input.name, normalized);
    });
